<?php
$conn = new mysqli("localhost", "marius", "Daten");
...
mysqli_query($connect, "DROP TABLE players");
mysqli_query($connect, "DROP TABLE roles");
mysqli_query($connect, "DROP TABLE doors");
mysqli_query($connect, "DROP TABLE game");
...
mysqli_query($connect, "CREATE TABLE players (RoomID int NOT NULL, PlayerID int NOT NULL, Name varchar(255) NOT NULL, Status varchar(255), Role varchar(255), SpielerMenu boolean, LinkMenu boolean, AmZug boolean, CardsSelected boolean, Leer int, Gold int, Feuerfalle int, UpdateNecessary boolean, CONSTRAINT FinalID PRIMARY KEY (RoomID, PlayerID));");
